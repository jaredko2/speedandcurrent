...
module.exports = function (app) {
  ...
  function getLearningStatePayload() {
    ...
    return {
      ...
      navigationState,
      tableDimension: options.tableDimension || 'heel'
    };
  }
  ...
  plugin.start = (settings) => {
    ...
    smoothedWindAngle = new SmoothedAngle(app, plugin.id, 'windAngleApparent', 'environment.wind.angleApparent', {
      angleRange: '-piToPi',
      meta: { displayName: 'Apparent wind angle', plane: 'Boat' },
      SmootherClass,
      smootherOptions,
      ...buildLifecycleCallbacks(
        'windAngleApparent.angle',
        () => smoothedWindAngle?.handler?.path || 'environment.wind.angleApparent',
        () => { smoothedWindAngle?.unsubscribe(); smoothedWindAngle?.subscribe(false, true); }
      )
    });
    rawWindAngle = smoothedWindAngle.handler;
    ...
    if (options.tableDimension === 'windApparent') {
      reportFull.addDelta(smoothedWindAngle);
    }
  ...
  plugin.stop = () => {
    return new Promise((resolve, reject) => {
      try {
        ...
        smoothedWindAngle = smoothedWindAngle?.terminate();
  ...
  function correct(wellUnderway) {
    ...
    const useWindDimension = options.tableDimension === 'windApparent';
    const dimensionReady = useWindDimension ? rawWindAngle?.ready : rawAttitude?.ready;
    const dimensionValue = useWindDimension ? rawWindAngle?.value : rawAttitude?.value?.roll;
    ...
    else if (dimensionReady) {
      if (correctedBoatSpeed.magnitude > 0) {
        const { correction, variance } = table.getCorrection(correctedBoatSpeed.magnitude, dimensionValue);
  ...
  function updateTable() {
    ...
    const useWindDimension = options.tableDimension === 'windApparent';
    const dimensionReady = useWindDimension ? smoothedWindAngle.ready : smoothedAttitude.ready;
    const dimensionValue = useWindDimension ? smoothedWindAngle.value : smoothedAttitude.value?.roll;
    const inputsReady = smoothedBoatSpeed.ready && smoothedHeading.ready && smoothedGroundSpeed.ready && dimensionReady;
    ...
    table.update(smoothedBoatSpeed.value, dimensionValue, smoothedGroundSpeed, options.assumeCurrent ? smoothedCurrent : noCurrent, lrnBoatSpeed, smoothedHeading.value);
  ...
  function applyOptionChanges() {
    ...
    if (changedKeys.some(k => SMOOTHER_KEYS.includes(k))) {
      ...
      if (smoothedWindAngle) { smoothedWindAngle.setSmootherClass(SC); smoothedWindAngle.setSmootherOptions(so); }
    }
    saveOptions();
  }
